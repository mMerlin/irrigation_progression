...
const int TRIGGER_PERCENT = 30; //pump is triggered to water below this value

const int WATERING_TIME = 2500; // the number of milliseconds for the motor to turn on for
const int SOAK_TIME = 5000;     // the minimum delay time to allow water to soak in before next watering session
...
void setup() {
  ...
  SoilSensorInit();
  PumpInit();
  monitoringState = CHECKING_MOISTURE;
}
  ...
  switch (monitoringState) {
    case CHECKING_MOISTURE:
      if (getMoisturePercent() < TRIGGER_PERCENT) {
        targetMillis = startWaterDelivery(stateMillis);
        monitoringState = DELIVERING_WATER;
      }
      break;
    case DELIVERING_WATER:
      if (stateMillis >= targetMillis) {
        targetMillis = stopWaterDelivery(stateMillis);
        monitoringState = SOAKING_IN;
      }
      break;
    case SOAKING_IN:
      if (stateMillis >= targetMillis) {
...
unsigned long startWaterDelivery(unsigned long startMarker)
{
  Serial.println("Turning pump on");
  pumpOn();
  return startMarker + WATERING_TIME;
}

unsigned long stopWaterDelivery(unsigned long startMarker)
{
  Serial.println("Turning pump off");
  pumpOff();               // let the water soak in
  return startMarker + SOAK_TIME;
}
