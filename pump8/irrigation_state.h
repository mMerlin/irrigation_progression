...
enum irrigation_state_t {
  ...
  WATERING_BLOCKED,
  /// water resources locked too long
...
extern const unsigned long POWER_WAIT_TIMEOUT;

bool takePowerToken(void);
void releasePowerToken(void);
void whenMoistureGood(irrigation_context_t *, smart_time_t);
void whenTooDry(irrigation_context_t *, smart_time_t);
void whenWateringBlocked(irrigation_context_t *, smart_time_t);
void whenResourceTimeout(irrigation_context_t *, smart_time_t);
void whenStartWatering(irrigation_context_t *, smart_time_t);
